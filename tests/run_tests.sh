...
if [[ -v COMPUTECPP_DIR ]]; then
  TOKENSTRING="$COMPUTECPP_DIR/bin/compute++ -sycl -O2 -mllvm -inline-threshold=1000 -intelspirmetadata -Wno-unused-command-line-argument -sycl-driver -sycl-target spir64 -no-serial-memop -I $COMPUTECPP_DIR/include -std=c++17 -DSYCL_LANGUAGE_VERSION=202001 -L $COMPUTECPP_DIR/lib -lComputeCpp %s"
  doit
fi
